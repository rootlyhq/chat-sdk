...
- `message.author.system?` -- Whether the message was generated by the platform
- `message.author.email` -- Normalized email when the adapter provides one
...
- `message.reply_to` -- Normalized message being replied to, when available

Message handlers may accept an optional third `context` argument. Queueing strategies populate `context[:skipped]` and `context[:total_since_last_handler]` when multiple overlapping messages are collapsed.

## Message lifecycle

Slack message edits and deletes are exposed as lifecycle callbacks:

```ruby
bot.on_message_updated do |thread, message, previous_message|
  puts "#{previous_message&.text} -> #{message.text}"
end

bot.on_message_deleted do |event|
  puts "Deleted #{event.message_id} from #{event.channel_id}"
end
```
...
|---------|----------------|---------------|
| `on_new_mention` | `\|thread, message, context\|` | Bot is @-mentioned |
| `on_new_message(pattern)` | `\|thread, message\|` | @-mention text matches pattern |
| `on_subscribed_message` | `\|thread, message, context\|` | Message in a subscribed thread |
| `on_direct_message` | `\|thread, message, context\|` | Direct message to the bot |
| `on_message_updated` | `\|thread, message, previous_message\|` | Existing message edited |
| `on_message_deleted` | `\|event\|` | Existing message deleted |
