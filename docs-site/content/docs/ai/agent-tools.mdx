...
tools = ChatSDK::AI.create_tools(preset: :messenger, require_approval: false)
```
...
Arguments accept both symbol and string keys. The `chat:` parameter is required for the executor (it needs access to the adapters to dispatch calls).
