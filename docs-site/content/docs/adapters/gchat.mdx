...
description: Google Chat API adapter with Card v2 via the chat_sdk-gchat gem.
---
...
For apps using [Google Workspace Events](https://developers.google.com/workspace/events) with a Cloud Pub/Sub subscription, use `parse_pubsub_event` to decode incoming push messages. The Chat event payload is base64-encoded inside the Pub/Sub envelope's `message.data` field:
