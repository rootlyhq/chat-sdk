...
description: Linear adapter with GraphQL API, HMAC-SHA256 webhook signature verification, issue comments, and reactions via the chat_sdk-linear gem.
---
...
|------------|-----------|-------|
| `reactions` | Yes | Emoji reactions on comments |
...
| `direct_messages` | No | Raises `NotSupportedError` |
...
Linear mentions use the `@username` format. Pass the username to `mention`:
