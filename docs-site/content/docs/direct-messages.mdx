...
description: Open DM conversations and send private messages to individual users across all supported platforms.
---
...
| Mattermost | Yes -- creates a direct channel |
| Discord | Yes -- creates a DM channel |
| Telegram | Yes -- sends to chat ID |
| Twilio | Yes -- sends to phone number |
| Messenger | Yes -- sends to user PSID |
| WhatsApp | Yes -- sends to phone number |

All nine platform adapters declare the `:direct_messages` capability. If you build a custom adapter without DM support, calling `open_dm` raises `ChatSDK::NotSupportedError`.
