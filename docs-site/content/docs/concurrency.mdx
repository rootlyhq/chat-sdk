---
title: Concurrency
description: Per-thread locking, event deduplication, and multi-process safety for concurrent event handling.
---
...
The deduplication key is derived from the event's message ID (for mention/subscribed\_message events) or event\_id from the raw payload.

## Multi-Process Safety

For multi-process deployments (multiple dynos, Puma workers, Sidekiq processes), use a persistent state backend: `ChatSDK::State::Redis`, `ChatSDK::State::Pg`, or `ChatSDK::State::Mysql`. All three provide atomic operations for locks and deduplication records.
